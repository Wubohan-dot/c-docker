...
int capabilities()
{
	...
	for (size_t i = 0; i < num_caps; i++) {
		// process control, PR_CAPBSET_DROP for drop capabilities
		//如果调用线程在其用户名称空间中具有CAP_SETPCAP功能，则从调用线程的功能边界集（bounding set）中删除arg2指定的功能。调用线程的所有子代都将继承新缩小的边界集。  bset?
	...
	cap_t caps = NULL;
	if (!(caps = cap_get_proc())	//return the capability state of the calling process, return cap_t: 
									//A cap_t holds information about the capabilities in each of the three sets, Permitted, Inheritable, and Effective.
		// 		int cap_set_flag(cap_t cap_p, cap_flag_t flag, int ncap, const cap_value_t *capsint ncap, cap_flag_value_t value );
		//	sets the flag, flag, of each capability in the array caps in the capability state identified by cap_p to value. 
		//	The argument, ncap, is used to specify the number of capabilities in the array, caps.
		//	clearing the inheritable set
...
int mounts(struct child_config *config)
{
	...
	// generate a unique directory by replacing XXXXXX
	if (!mkdtemp(mount_dir)) {
		fprintf(stderr, "failed making a directory!\n");
		return -1;
	}
	// 把 config->mount_dir（-m ~/misc/busybox-img/） 挂载到mount_dir上，flag（VFS的通用选项为两个），data（该文件系统的通用选项）为NULL
	...
	char *old_root_dir = basename(inner_mount_dir); //-> oldroot.009X36
	char old_root[sizeof(inner_mount_dir) + 1] = { "/" }; //-> //////////////////////////////
	strcpy(&old_root[1], old_root_dir); //old_root-> /oldroot.009X36, 即是套中套

	fprintf(stderr, "=> unmounting %s...", old_root);
	// change current working directory to argument path
	if (chdir("/")) {
...
// resources: use cgroup mechanism to limit source
int resources(struct child_config *config)
{
	fprintf(stderr, "=> setting cgroups...");
	// for every cgrp in cgrps, 
	for (struct cgrp_control **cgrp = cgrps; *cgrp; cgrp++) {
		char dir[PATH_MAX] = {0};
		fprintf(stderr, "%s...", (*cgrp)->control);
		// make directory 	/sys/fs/cgroup/control_issue/hostname   to write in settings
		// such as /sys/fs/cgroup/cpu/wbh_hhh_1022
		...
		for (struct cgrp_setting **setting = (*cgrp)->settings; *setting; setting++) {
			...
			int fd = 0;
			//	/sys/fs/cgroup/control_issue/hostname/setting_name and write value
			//  such as: write 256(SHARE) /sys/fs/cgroup/cpu/wbh_hhh_1022/cpu.shares
...
// sockets[0] in function 'main' for fd
int handle_child_uid_map (pid_t child_pid, int fd)
{
	...
	// then the procedure 'userns' can get its result from sockets[1]
...
int userns(struct child_config *config)
{
	...
	// transport message 'has_userns' to socket, then parent process call function 'handle_child_uid_map'
	if (write(config->fd, &has_userns, sizeof(has_userns)) != sizeof(has_userns)) {
		fprintf(stderr, "couldn't write: %m\n");
		return -1;
	}
	int result = 0;
	// get blocked until handle_child_uid_map read from sockets[0] and process it and put new message
...
int child(void *arg)
{
	...
	if (execve(config->argv[0], config->argv, NULL)) {
		fprintf(stderr, "execve failed! config->argv[0]:%s...%m.\n", config->argv[0]);
		return -1;
	}
	return 0;
}
...
int main (int argc, char **argv)
{
	...
	int err = 0;
	int option = 0;
	// 2 flags for two thread hodling each side
	int sockets[2] = {0};
	pid_t child_pid = 0;
	int last_optind = 0;
	// getopt: analyze command arguments one by one
	// get config
...
finish_options:
	...
	// nearly randomly get generate a hostname
	char hostname[256] = {0};
	if (choose_hostname(hostname, sizeof(hostname)))
		goto error;
	config.hostname = hostname;

	// a socket to pass uid, hostname ...
	if (socketpair(AF_LOCAL, SOCK_SEQPACKET, 0, sockets)) {
		fprintf(stderr, "socketpair failed: %m\n");
		goto error;
	}
	// here use fcntl to keep the socket close after child process begin exec
	...
	// malloc space for child process as stack
	char *stack = 0;
	...
	// use cgroup mechanism to limit resource for child process
	if (resources(&config)) {
		err = 1;
		goto clear_resources;
	}

	// clone to fork child process
	int flags = CLONE_NEWNS
		...
		| CLONE_NEWNET
		| CLONE_NEWUTS;
	// use flag SIGCHLD to prevent zoombie process
	...
	// pass child pid to child process to implement map
